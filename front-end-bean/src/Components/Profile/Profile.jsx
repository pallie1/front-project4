    ...
    if (activeUser.reviews.length === 0) {
      return (
        <>
          ...
          <Link className="button-class" to="/edit-profile">
            Edit Profile
          </Link>
      ...
      let usersRevs = activeUser.reviews.map((rev) => {
        for (let i = 0; i < activeUser.reviews.length; i++) {
          if (activeUser.shops[i].id === rev.shop_id) {
            // cafeInfo.push(activeUser.shops[i].name, activeUser.shops[i].id);

        return (
          <div key={rev.id} className="user-rev-div">
            <div>
              <Link to={`/reviews/${activeUser.shops[i].id}`}>
                <h3 className="name-feed">{activeUser.shops[i].name}</h3>
              </Link>
            </div>
            <div>
              {rev.img.length < 5 ? null : (
                <img src={rev.img} alt="user submited coffee shop" />
              )}
              <p>{rev.rate}</p>
              <p>{rev.content}</p>
            </div>
...
//           <div key={rev.id} className="user-rev-div">
//             <div>
//               <Link to={`/reviews/${cafeInfo[1]}`}>
//                 <h3 className="name-feed">{cafeInfo[0]}</h3>
//               </Link>
//             </div>
//             <div>
//               {rev.img.length < 5 ? null : (
